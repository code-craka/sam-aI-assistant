...
class ChatManager: ObservableObject {
    ...
    func sendMessageWithStreaming(_ content: String, threadId: UUID? = nil) -> AsyncThrowingStream<
        String, Error
    > {
    ...
    func editMessage(_ messageId: UUID, newContent: String) async {
        ...
        if let conversation = currentConversation {
            do {
                try await chatRepository.updateMessage(
                    messageId, content: newContent, in: conversation)
            } catch {
    ...
    func loadConversation(_ conversation: ChatModels.Conversation) async {
        ...
        do {
            ...
            if let coreDataConversation = try await chatRepository.getConversation(
                id: conversation.id)
            {
                currentConversation = coreDataConversation

                // Load messages for this conversation
                let conversationMessages = try await chatRepository.getMessages(
                    for: coreDataConversation)
    ...
    private func createNewConversation() async {
        do {
            ...
            let welcomeMessage = try await chatRepository.addAssistantMessage(
                content:
                    "Hello! I'm Sam, your macOS AI assistant. I can help you with file operations, system queries, app control, and much more. What would you like me to help you with today?",
                taskType: .help,
                to: conversation
            )
    ...
    private func streamResponse(
        _ response: String, continuation: AsyncThrowingStream<String, Error>.Continuation
    ) async {
    ...
    private func processMessage(_ content: String) async throws -> ProcessedMessageResult {
        ...
        // Process the message through the new task router
        let routingResult = try await taskRouter.processInput(content)

        let executionTime = Date().timeIntervalSince(startTime)

        // Convert routing result to legacy task result format for compatibility
        let taskResult = TaskResult(
            success: routingResult.success,
            output: routingResult.output,
            executionTime: routingResult.executionTime,
            errorMessage: routingResult.error?.localizedDescription
        )

        return ProcessedMessageResult(
            output: routingResult.output,
            taskType: routingResult.classification.taskType,
            taskResult: taskResult,
            executionTime: executionTime,
            tokens: routingResult.tokensUsed,
    ...
    private func inferTaskType(from input: String, result: TaskResult) -> TaskType {
        ...
        if lowercased.contains("copy") || lowercased.contains("move")
            || lowercased.contains("delete") || lowercased.contains("file")
        {
            return .fileOperation
        } else if lowercased.contains("battery") || lowercased.contains("storage")
            || lowercased.contains("memory")
        {
            return .systemQuery
        } else if lowercased.contains("open") || lowercased.contains("app")
            || lowercased.contains("safari")
        {
            return .appControl
        } else if lowercased.contains("help") || lowercased.contains("what")
    ...
    private func updateUsageMetrics(with message: ChatModels.ChatMessage) {
        usageMetrics = ChatModels.UsageMetrics(
            ...
            successfulTasks: usageMetrics.successfulTasks
                + (message.taskResult?.success == true ? 1 : 0),
